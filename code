...
# Adding a new column. If the value of the float column is not "no value", then we cut off the first character (which is a dollar sign), and replace all commas with nothing, and then cast that value to a float. If the answer was "no value", then we just enter a 0.
